(function () {
  ...
  const keys = {
    ...
    syncConfig: `${prefix}_sync_config`,
  };
  ...
  function normalizeSyncMethod(value) {
    return String(value || "PUT").trim().toUpperCase() === "POST" ? "POST" : "PUT";
  }

  function normalizeTimeout(value) {
    const timeout = Number(value);
    return Number.isFinite(timeout) ? Math.max(2000, timeout) : 8000;
  }
  ...
  function normalizeSyncConfig(input) {
    ...
    return {
      ...
      readUrl: String(source.readUrl || "").trim(),
      writeUrl: String(source.writeUrl || "").trim(),
      method: normalizeSyncMethod(source.method),
      authHeader: String(source.authHeader || "").trim(),
      authToken: String(source.authToken || "").trim(),
      requestTimeoutMs: normalizeTimeout(source.requestTimeoutMs)
    };
  }

  function normalizeSyncConfigOverride(input) {
    const source = input && typeof input === "object" && !Array.isArray(input) ? input : {};
    const normalized = {};
    ...
    if (Object.prototype.hasOwnProperty.call(source, "readUrl")) {
      normalized.readUrl = String(source.readUrl || "").trim();
    }

    if (Object.prototype.hasOwnProperty.call(source, "writeUrl")) {
      normalized.writeUrl = String(source.writeUrl || "").trim();
    }

    if (Object.prototype.hasOwnProperty.call(source, "method")) {
      normalized.method = normalizeSyncMethod(source.method);
    }

    if (Object.prototype.hasOwnProperty.call(source, "authHeader")) {
      normalized.authHeader = String(source.authHeader || "").trim();
    }

    if (Object.prototype.hasOwnProperty.call(source, "authToken")) {
      normalized.authToken = String(source.authToken || "").trim();
    }

    if (Object.prototype.hasOwnProperty.call(source, "requestTimeoutMs")) {
      normalized.requestTimeoutMs = normalizeTimeout(source.requestTimeoutMs);
    }
  ...
  function getSyncConfig() {
    const defaults = normalizeSyncConfig(site.sync || {});
    const override = normalizeSyncConfigOverride(readJSON(keys.syncConfig, {}));
    return { ...defaults, ...override };
  }

  function saveSyncConfig(config) {
    writeJSON(keys.syncConfig, normalizeSyncConfig(config));
  }
  ...
  async function syncMediaToCloud() {
    ...
    try {
      await pushCloudMapNow(getMediaMap());
      return { ok: true };
    } catch (_error) {
      warnCloudSyncFailure();
      return { ok: false, skipped: false };
