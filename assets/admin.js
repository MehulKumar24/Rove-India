(function () {
  ...
  document.addEventListener("DOMContentLoaded", () => {
    ...
    const syncReadUrlInput = document.getElementById("syncReadUrl");
    const syncWriteUrlInput = document.getElementById("syncWriteUrl");
    const syncMethodInput = document.getElementById("syncMethod");
    const syncTimeoutInput = document.getElementById("syncTimeout");
    const syncAuthHeaderInput = document.getElementById("syncAuthHeader");
    const syncAuthTokenInput = document.getElementById("syncAuthToken");
    const resetSyncConfigBtn = document.getElementById("resetSyncConfig");
    ...
    function getSyncConfigState() {
      if (typeof utils.getSyncConfig === "function") {
        return utils.getSyncConfig();
      }
    ...
    function renderSyncSettings() {
      if (!syncConfigForm) return;

      const config = getSyncConfigState();

      if (syncEnabledInput) syncEnabledInput.checked = Boolean(config.enabled);
      if (syncReadUrlInput) syncReadUrlInput.value = config.readUrl || "";
      if (syncWriteUrlInput) syncWriteUrlInput.value = config.writeUrl || "";
      if (syncMethodInput) syncMethodInput.value = config.method === "POST" ? "POST" : "PUT";
      if (syncTimeoutInput) syncTimeoutInput.value = String(config.requestTimeoutMs || 8000);
      if (syncAuthHeaderInput) syncAuthHeaderInput.value = config.authHeader || "";
      if (syncAuthTokenInput) syncAuthTokenInput.value = config.authToken || "";
    }

    function updateSyncStatusNote() {
      if (!syncStatusNote) return;

      const sync = getSyncConfigState();
      const enabled = Boolean(sync.enabled);
      const readUrl = String(sync.readUrl || "").trim();
      const writeUrl = String(sync.writeUrl || "").trim();

      syncStatusNote.hidden = false;
      syncStatusNote.className = "status-msg";

      if (!enabled) {
        syncStatusNote.textContent =
          "Cloud sync is off. Enable it in Cloud Sync Settings after login to share uploads across devices.";
        return;
      }

      if (readUrl && writeUrl) {
        syncStatusNote.classList.add("success");
        syncStatusNote.textContent = "Cloud sync is active. Media updates can be shared across devices.";
        return;
      }

      syncStatusNote.classList.add("warn");
      syncStatusNote.textContent =
        "Cloud sync is enabled but incomplete. Add both read and write URLs in Cloud Sync Settings to share media across devices.";
    }
    ...
    function validateSyncConfig(config) {
      ...
      if (!config.readUrl || !config.writeUrl) {
        utils.notify("Add both read and write URLs before enabling cloud sync.", "warn");
        return false;
      }

      if (!/^https?:\/\//i.test(config.readUrl) || !/^https?:\/\//i.test(config.writeUrl)) {
        utils.notify("Sync URLs must start with http or https.", "warn");
        return false;
      }

      if (config.authToken && !config.authHeader) {
    ...
    async function handleSyncConfigSave(event) {
      event.preventDefault();

      const config = {
        enabled: Boolean(syncEnabledInput?.checked),
        readUrl: (syncReadUrlInput?.value || "").trim(),
        writeUrl: (syncWriteUrlInput?.value || "").trim(),
        method: syncMethodInput?.value === "POST" ? "POST" : "PUT",
        authHeader: (syncAuthHeaderInput?.value || "").trim(),
        authToken: (syncAuthTokenInput?.value || "").trim(),
        requestTimeoutMs: Math.max(2000, Number(syncTimeoutInput?.value || 8000))
      };
      ...
      if (typeof utils.saveSyncConfig === "function") {
        utils.saveSyncConfig(config);
      }
      ...
      utils.notify(
        config.enabled ? "Cloud sync settings saved." : "Cloud sync disabled. Media changes will stay local on this device.",
        "success"
      );
    }

    function handleSyncConfigReset() {
      if (typeof utils.clearSyncConfig === "function") {
        utils.clearSyncConfig();
      }
      ...
      utils.notify("Cloud sync settings reset to project defaults.", "warn");
    }

    async function handleSyncNow() {
      if (!isLoggedIn()) {
        utils.notify("Login first to run cloud sync.", "warn");
        return;
      }

      if (typeof utils.hasCloudSync !== "function" || !utils.hasCloudSync()) {
        utils.notify("Complete Cloud Sync Settings before running sync.", "warn");
        return;
      }
      ...
      try {
        const pushResult =
          typeof utils.syncMediaToCloud === "function" ? await utils.syncMediaToCloud() : { ok: false, skipped: true };
        const pullResult = typeof utils.syncMediaFromCloud === "function" ? await utils.syncMediaFromCloud() : { ok: false, skipped: true };

        populatePlaceOptions();
        touchSession();

        if (pushResult.ok || pullResult.ok) {
          utils.notify("Cloud sync completed.", "success");
        } else {
          utils.notify("Cloud sync could not connect. Check the configured URLs and token.", "warn");
        }
      } finally {
        if (syncNowBtn) {
    ...
    syncConfigForm?.addEventListener("submit", handleSyncConfigSave);
    resetSyncConfigBtn?.addEventListener("click", handleSyncConfigReset);
